...
def withdraw_start(message):
    ...
    if pending == 1:
        bot.send_message(
            message.chat.id,
            "⏳ شما یک برداشت در حال بررسی دارید\n"
            "❌ تا تعیین وضعیت، امکان ثبت درخواست جدید نیست",
            reply_markup=main_menu()
        )
        return
...
def withdraw_get_link(message):
    ...
    data = withdraw_requests.get(uid)
    if not data:
    ...
    # قفل برداشت
    cur.execute(
        "UPDATE users SET pending_withdraw=1 WHERE user_id=?",
        (uid,)
    )
    db.commit()
...
def reject_withdraw(c):
    ...
    cur.execute(
        "UPDATE users SET pending_withdraw=0 WHERE user_id=?",
        (uid,)
    )
    db.commit()
