...
try:
    ...
    pass
    # افزودن ستون captcha_passed اگر وجود ندارد
try:
    cur.execute("ALTER TABLE users ADD COLUMN captcha_passed INTEGER DEFAULT 0")
    db.commit()
except:
    pass
...
ensure_column("users", "invited_by", "INTEGER")
ensure_column("users", "captcha_passed", "INTEGER DEFAULT 0")
...
broadcast_data = {}
# ===== محدودیت گروه =====
ALLOWED_GROUP_ID = -1003529474317   # آیدی گروه مجاز
ADMINS = [8589848955, 111111111]    # آیدی عددی ادمین‌ها
# ================= منو =================
...
def block_group_users(message):
    # اگر گروه مجاز نیست → هیچی
    if message.chat.id != ALLOWED_GROUP_ID:
        return

    # اگر ادمین نیست → بلاک کامل
    if not is_admin(message.from_user.id):
        # حتی جواب هم نده (سایلنت)
        return
...
@bot.message_handler(
    ...
    func=lambda m: (
        ...
        and m.chat.id == ALLOWED_GROUP_ID
        and is_admin(m.from_user.id)
    )
)
def stars_admin_only(message):
    bot.reply_to(
        message,
        "⭐ این دستور فقط برای ادمین‌ها فعال است"
    )
@bot.message_handler(
    commands=['start'],
...
def broadcast_confirm(call):
    ...
    users = get_all_users()  # لیست user_id ها
