...
db.commit()
try:
    cur.execute("ALTER TABLE users ADD COLUMN pending_withdraw INTEGER DEFAULT 0")
    db.commit()
except:
    pass
    
try:
    cur.execute("ALTER TABLE users ADD COLUMN last_transfer INTEGER DEFAULT 0")
    db.commit()
except:
    pass
...
def check_captcha(message):
    ...
    if inviter:
        ...
        cur.execute(
            "UPDATE users SET balance=balance+?, invite_count=invite_count+1 WHERE user_id=?",
            (reward, inviter)
        )
...
@bot.message_handler(
    func=lambda m: (
        ...
        and TRANSFER_GROUP_ID == m.chat.id
...
def group_transfer(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ فرمت صحیح: انتقال 1.5")
        return
...
amount = 0.1  # مقدار انتقال

text = f"""
💰 مقدار {amount} STARS  
👤 از: {user_tag(sender)} (ID: {sender.id})  
👤 به: {user_tag(receiver)} (ID: {receiver.id})  
...
def withdraw_start(message):
    ...
    cur.execute("SELECT balance, pending_withdraw FROM users WHERE user_id=?", (uid,))
    bal, pending = cur.fetchone()

    if pending == 1:
        bot.send_message(
            message.chat.id,
            "⏳ یک برداشت در حال بررسی داری\n❌ تا تعیین وضعیت نمی‌تونی برداشت جدید ثبت کنی"
        )
        return

    if bal <= 0:
        bot.send_message(message.chat.id, "❌ موجودی صفره", reply_markup=main_menu())
        return
...
def withdraw_get_amount(message):
    ...
    try:
        ...
        msg = bot.send_message(message.chat.id, "❌ مقدار نامعتبر", reply_markup=back_menu())
        bot.register_next_step_handler(msg, withdraw_get_amount)
        return
    ...
    if amount > bal:
        withdraw_requests.pop(uid, None)
        bot.send_message(message.chat.id, "❌ موجودی کافی نیست", reply_markup=main_menu())
        return
...
def withdraw_get_link(message):
    ...
    cur.execute("UPDATE users SET pending_withdraw=1 WHERE user_id=?", (uid,))
    db.commit()
...
def approve_withdraw(c):
    ...
    cur.execute(
        "UPDATE users SET balance=balance-?, order_count=order_count+1, pending_withdraw=0 WHERE user_id=?",
        (amount, uid)
    )
    db.commit()

    bot.edit_message_text("✅ برداشت تأیید شد", c.message.chat.id, c.message.message_id)
    bot.send_message(uid, f"🎉 برداشتت تأیید شد\n⭐ مقدار: {amount}")
...
def reject_withdraw(c):
    ...
    cur.execute("UPDATE users SET pending_withdraw=0 WHERE user_id=?", (uid,))
    db.commit()

    bot.edit_message_text("❌ برداشت رد شد", c.message.chat.id, c.message.message_id)
    bot.send_message(uid, "❌ برداشتت رد شد\n📞 با پشتیبانی تماس بگیر")
